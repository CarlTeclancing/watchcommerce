...
<head>
    ...
    <style>
        :root { --luxury-gold: #d4af37; --luxury-dark: #0f172a; --ink: #0b1220; --ring: rgba(212,175,55,0.35); }
        ...
        .cart-badge { position: absolute; top: -0.45rem; right: -0.6rem; min-width: 1.15rem; height: 1.15rem; border-radius: 9999px; background: #d4af37; color: #0f172a; font-size: 0.7rem; line-height: 1.15rem; font-weight: 700; text-align: center; padding: 0 0.2rem; }
        .header-container { max-width: 1280px; margin: 0 auto; }

        /* ── Design system (shared with public pages) ── */
        a, button, input, select, textarea { transition: all 0.2s ease; }
        a:focus-visible, button:focus-visible, input:focus-visible, select:focus-visible, textarea:focus-visible {
            outline: none; box-shadow: 0 0 0 4px var(--ring); border-radius: 0.5rem;
        }
        .surface-card {
            background: #fff;
        ...
        .form-control:focus { outline: none; border-color: #d4af37; box-shadow: 0 0 0 3px var(--ring); }
        .form-control::placeholder { color: #94a3b8; }
        .btn-primary {
            display: inline-flex; align-items: center; gap: 0.4rem;
            background: linear-gradient(135deg, #f3c54a, #d4af37);
            color: #0f172a; border: 1px solid #c9a62f; border-radius: 0.65rem;
            font-weight: 700; padding: 0.6rem 1.2rem; cursor: pointer;
        ...
        .btn-secondary {
            ...
            border-radius: 0.65rem; font-weight: 600; padding: 0.6rem 1.2rem; cursor: pointer;
        }
...
<div class="trust-badge">
    <i class="fas fa-shield-halved"></i> <?= htmlspecialchars($heroBadgeText, ENT_QUOTES, 'UTF-8') ?>
</div>

<div class="header-top px-4 md:px-8">
    <div class="header-container w-full flex items-center justify-between">
        <div class="hidden sm:flex gap-4">
            <a href="tel:<?= htmlspecialchars($supportPhone, ENT_QUOTES, 'UTF-8') ?>" class="icon-link"><i class="fas fa-phone"></i></a>
        </div>
...
<div class="header-main px-4 md:px-8">
    <div class="header-container w-full">
        <div class="flex flex-wrap md:flex-nowrap items-center gap-4 md:gap-8">
            <a href="<?= $basePath ?? '' ?>/" class="header-logo text-4xl md:text-5xl flex items-center gap-2">
                <?php if ($logoUrl !== ''): ?>
                <img src="<?= htmlspecialchars($logoUrl, ENT_QUOTES, 'UTF-8') ?>" alt="<?= htmlspecialchars($siteName, ENT_QUOTES, 'UTF-8') ?>" class="h-10 w-auto object-contain">
                <?php else: ?>
